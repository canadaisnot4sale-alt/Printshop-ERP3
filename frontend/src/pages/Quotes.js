...
export default function Quotes() {
  ...
  return (
    <div data-testid="quotes-page">
      <PageHeader title="My Quotes" subtitle="Saved quotes — email or print as PDF">
        <Button onClick={() => window.print()} variant="outline" className="rounded-sm"><Printer size={15} className="mr-1.5" />PDF</Button>
      </PageHeader>
      <div className="p-8">
        {quotes.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-sm p-12 text-center text-slate-400">No saved quotes yet. Use "Save" on any calculator.</div>
        ) : (
      ...
      <Dialog open={emailOpen} onOpenChange={setEmailOpen}>
        <DialogContent className="rounded-sm">
          ...
          <Input data-testid="email-recipient" type="email" value={recipient} onChange={(e) => setRecipient(e.target.value)} placeholder="customer@email.com" className="rounded-sm" />
          <DialogFooter>
            <Button variant="outline" onClick={() => setEmailOpen(false)} className="rounded-sm">Cancel</Button>
            <Button data-testid="email-send-confirm" onClick={sendEmail} disabled={sending || !recipient} className="bg-[#2495D3] hover:bg-[#1E7AA9] rounded-sm">
