...
## Done (2026-06)
- **Storefront theme — Phase 1 (Dawn + brand)**: Clients/resellers (and admin via "View as") now get a dedicated e-commerce shell (`StoreLayout`) totally separate from the admin ERP — announcement bar, sticky header (logo, nav Home/Shop all/My orders, search, account menu, cart badge), brand-blue hero ("Browse our latest products" + Shop all), Products grid + categories (`StoreHome`), footer. `Protected` picks StoreLayout for customer roles, `HomeRouter` shows StoreHome at `/`. Admin gets an amber "Admin preview → Exit to Admin" bar. Verified via screenshot. Blueprint saved to `/app/design_guidelines.json`.
  - Phase 2 (customer self-signup + Google login + email verification + captcha) and Phase 3 (inactive-account auto-disable, wholesale approval) are NEXT — auth needs its integration playbooks. Public browsing with prices-behind-login also in Phase 2.
...
- **Edit configurable product + Marketing view/copy + Client order stages + PDF validation**:
  ...
  - **PDF template validation**: `/api/upload/file` detects PDF page size (pypdf); configurable products store `config.template` (finished + 0.25" bleed) which is copied onto order lines; on client artwork upload the Orders UI warns if the PDF size doesn't match the template (±0.15", allows rotation). NOTE: only checks dimensions, not DPI.
- **Client production files, file-setup fee, product images, marketing tone & reorder (Object Storage)**:
  - Generic upload `POST /api/upload/file` (auth, 50MB, pdf/png/jpg/tiff/ai/eps/svg) + soft-delete + `/api/files/{id}/download?auth=`.
  - **Order files**: `GET /api/orders/{id}`, `POST/DELETE /api/orders/{id}/files` — client uploads artwork, admin uploads production/proof files; both listed in the order dialog (Orders.js) with download; whole-order **Reorder** button.
  - **File-setup fee**: configurable product `config.file_handling.fee` (default $25, editable per product in Convert dialog). Storefront product page shows a "Your artwork" choice (print-ready free vs "I need setup +$fee"); fee added to the order server-side (verified total base+25). Warning shown for non-print-ready files.
  - **Product image upload** in Convert dialog → shown in store (auth'd image URLs).
  - **Marketing tone**: tone selector (Professional/Friendly/Playful/Luxury/Bold) on "Generate with AI" → passed to Claude.
  - Verified via curl (upload/attach/download client+proof, fee total 28.13, reorder 3.13, tone regen) + screenshots.
  - DEFERRED (next): full "Edit configurable product" (reopen to change papers/turnarounds/related) and a dedicated "View/Copy marketing" panel in Products page.
