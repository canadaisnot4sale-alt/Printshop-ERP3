...
- **Tax-Included view** (all modules): shared PricingPanel adds a "Tax Included" block — Retail + GST% + PST% (default 12%) and Wholesale + GST% only (default 5%). Rates read from settings.gst_pct/pst_pct via the same cached fetch as Rush (invalidated on Settings save). Display-only. Verified: Retail $19.38→$21.71, Wholesale $12.26→$12.87.
