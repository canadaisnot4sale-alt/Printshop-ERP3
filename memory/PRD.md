## Implemented (2026-06) — Financials Phase 2/3/4 (Best-sellers, Promote, Real profit per order)
- **Phase 2 — Rentabilidad & más vendidos**: `GET /api/finance/best-sellers?period=day|week|month|year` (admin) ranks products by units, revenue and REAL profit (revenue − BoM cost via compute_product_pricing). Products w/o BoM → cost_known=false (profit shown as "— sin costo", unit_profit/times_to_goal null). Each row returns times_to_goal = ceil(monthly_goal / unit_profit) ("vende ×N para llegar a la meta"). New `POST /api/finance/manual-sale` records off-system invoices as a completed order (source='manual') so external sales count in goals + best-sellers. Panel `SalesInsights.js` on /profit-dashboard (period selector, Más vendidos/Más rentables tabs, "Venta externa" dialog).
- **Phase 3 — Impulso a ventas**: "Promocionar" button per best-seller opens a share dialog (Facebook / WhatsApp / Email / Copy) prefilled with the product's AI marketing text (marketing.instagram/facebook/seo_description ES/EN) + storefront link.
- **Phase 4 — Ganancia real por orden**: `GET /api/orders/{oid}/pnl` (admin) returns quoted margin (BoM computed_cost×qty) vs REAL margin (actual material + ink deducted). `deduct_inventory_for_order` now stores per-deduction `cost` (unit_cost×total; laminate roll_cost×rolls); `deduct_order_ink` stores ink `cost` (ml×unit_cost/ink_volume_ml). Orders invoice dialog shows a P&L panel (Margen cotizado vs real + variance), refreshes after ink deduction. When quoted_known=false, quoted_margin/variance return null and UI shows "—" (no false warning).
- Verified iteration_35: backend 14/14 pytest; frontend 100% of flows. Fixed 2 logic issues post-test (null quoted margin/variance without BoM; null times_to_goal without cost). Backend curl-confirmed.
- NOTE: best-sellers counts orders in any status except 'cancelled' (incl. pending). server.py now 4639 lines — router split still pending (P0 refactor).
...
- **In-product-dialog video manager**: ALL THREE product editors now let you attach/name/remove multiple videos, fully synced. (1) ProductsCatalog "New/Edit product" dialog. (2) Paper Printing "Convert to product" dialog. (3) ProductsCatalog "Configure & marketing" dialog (configurable products, next to Related products + Marketing). Videos link via ref_id right after save. Verified all three flows.
...
- NOTE: There is NO auto-upload to YouTube (user decided 2026-08-20 to keep the simple paste-link flow — no daily quota limits, no Google Cloud setup). Option B (YouTube Data API upload + Google OAuth) explicitly declined for now. YouTube channel would have been under info@printandsave.net if revisited.
