...
- **In-product-dialog video manager**: BOTH product creation flows now let you attach/name/remove multiple videos. (1) ProductsCatalog "New/Edit product" dialog. (2) Paper Printing "Convert to product" dialog (configurable Grab-n-Go products) — "Training videos" section before "Publish now". Videos are created and linked (ref_id) right after the product saves. Verified both flows.
